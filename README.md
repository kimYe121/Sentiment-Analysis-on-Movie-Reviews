...
模型正则化治理（dropout 0.5→0.7、引入 AdamW 权重衰减 / 标签平滑 / 余弦学习率退火）作为一次完整的配置前后对比实验记录在实验结果节。项目另预留消融实验框架：在编排脚本 `GROUPS` 中按行添加"每次只改一个参数"的实验即可扩展。
...
**主对比表**（stratified 验证集 15,606 条；多数类基线 accuracy = 0.512；以 `results/comparison.csv` 为准）：

| 模型 | 划分 | accuracy | macro F1 | weighted F1 |
|---|---|---|---|---|
| **BERT（微调）** | stratified | **0.7051** | **0.6304** | **0.7057** |
| BiLSTM + Attention（手写） | stratified | 0.6756 | 0.5759 | 0.6690 |
| TextCNN（手写） | stratified | 0.6723 | 0.5588 | 0.6649 |
| Logistic Regression | stratified | 0.6615 | 0.5466 | 0.6501 |
| LinearSVC | stratified | 0.6523 | 0.5404 | 0.6427 |
| Random Forest | stratified | 0.6323 | 0.5166 | 0.6200 |
| Multinomial NB | stratified | 0.6323 | 0.4823 | 0.6140 |
| 三模型概率集成 | stratified | 0.7015 | 0.6078 | 0.6971 |
| BiLSTM（nn.LSTM 对照） | stratified | 0.6738 | 0.5666 | 0.6681 |
| BiLSTM + Attention（手写） | grouped | 0.6272 | 0.4746 | 0.6077 |
| TextCNN（手写） | grouped | 0.6085 | 0.4294 | 0.5857 |
| Logistic Regression | grouped | 0.6038 | 0.4587 | 0.5845 |
| LinearSVC | grouped | 0.5940 | 0.4526 | 0.5742 |
| Multinomial NB | grouped | 0.5695 | 0.3113 | 0.5031 |
| Random Forest | grouped | 0.5536 | 0.3128 | 0.4837 |

**结果解读要点**：① 模型排序：预训练微调（BERT）> 从零训练的深度模型（BiLSTM ≈ TextCNN）> 线性经典模型（LR / SVC）> 概率/树模型（NB / RF）> 多数类基线；② **泄漏效应的差异化影响**（本项目最重要的方法学发现）：分层 vs 分组的落差并不均匀——线性模型与深度模型的 accuracy 跌 5~6.5pt，而 **NB / RF 的 macro F1 崩落 17~20pt**（RF 0.517→0.313）：非线性经典模型从短语重叠泄漏中获益最大，其分层口径下的少数类表现大部分是重叠记忆的产物；③ **线性经典模型非常有竞争力**：LR 分层成绩距 TextCNN 仅 1.1pt，grouped 口径下两者 accuracy 几乎持平（0.604 vs 0.609）且 LR 的 macro F1 反超（0.459 vs 0.429）——稀疏 TF-IDF 特征上线性方法性价比最高（秒级训练），树模型最差（RF 训练 7.6 分钟且 grouped 后垫底），与高维稀疏文本场景的文献结论一致；④ 手写 BiLSTM 与 nn.LSTM 精度等价（0.6756 vs 0.6738）而速度差约 19 倍，差距源于框架级算子融合而非算法本身；⑤ 三模型概率集成（0.7015）低于最佳单模 BERT（0.7051）——成员能力差距大、错误模式高度相关，简单平均被弱成员拉低；⑥ 上下文融合（BiLSTM 拼接式、BERT 句对式）均未超越基线——SST 标注以短语为单位，短片段反被语境误导；⑦ 正则化治理（dropout 0.7/0.6 + AdamW + 标签平滑 + 余弦退火）将有效训练从第 3 轮延长到第 9/6 轮、精度持平——15.6 万条短语仅来自 8.5 千个句子，饱和后继续训练即过拟合。
