...
|---|---|---|---|
| TextCNN（手写） | 0.6750 | 0.5634 | 0.6653 |
| BiLSTM + Attention（手写） | 0.6753 | 0.5851 | 0.6704 |
| **BERT（微调）** | **0.7044** | **0.6296** | **0.7045** |
| BiLSTM（nn.LSTM 对照） | 0.6730 | 0.5761 | 0.6697 |
| TextCNN（grouped 划分） | 0.6099 | 0.4272 | 0.5786 |
| BiLSTM（grouped 划分） | 0.6263 | 0.4985 | 0.6128 |

**结果解读要点**：① 训练损失全程下降而验证指标第 2~3 轮见顶回落，说明模型是**过拟合**而非拟合能力不足，早停保留的是最优轮权重；② macro F1 系统性低于 accuracy 源自类别不均衡；③ 分层与分组划分的落差（5~6.5pt）量化了短语重叠泄漏的影响。
...
- ✅ base / nn_compare / grouped 实验全部完成（上表）
- ✅ 可视化：EDA 图、指标柱状图、训练曲线、混淆矩阵、注意力热力图
